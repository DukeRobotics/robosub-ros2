...
class PeripheralPublisher(SerialRepublisherNode):
    ...
    def __init__(self):
        ...
        self._pub_depth = self.Publisher(DEPTH_DEST_TOPIC, PoseWithCovarianceStamped, 50)
        self._pub_voltage = self.Publisher(VOLTAGE_DEST_TOPIC, Float64, 10)
        ...
        self._pub_temperature = self.Publisher(TEMPERATURE_DEST_TOPIC, Float64, 10)
        self._pub_humidity = self.Publisher(HUMIDITY_DEST_TOPIC, Float64, 10)

        self._servo_service = self.Service(SERVO_SERVICE, SetBool, self.servo_control)
