...
class Sonar:
    ...
    def __init__(self):
        self.init_node(self.NODE_NAME)
        self.stream = self.get_param('~stream')
        self.debug = self.get_param('~debug')

        self.status_publisher = self.Publisher(String, self.SONAR_STATUS_TOPIC,  queue_size=1)
        self.pub_response = self.Publisher(SonarSweepResponse, self.SONAR_RESPONSE_TOPIC,  queue_size=1)

        # Enamble streaming
        self.cv_bridge = CvBridge()
        if self.stream:
            self.sonar_image_publisher = self.Publisher(CompressedImage, self.SONAR_IMAGE_TOPIC, queue_size=1)
    ...
    def init_sonar(self):
        ...
        try:
            ...
            self.logerr("Sonar not found.")
            self.signal_shutdown("Shutting down sonar node.")
        ...
        while not self.is_shutdown():
            try:
                ...
                self.logerr(f"Failed to connect to sonar: {e}")
    ...
    def constant_sweep(self, start_angle, end_angle, distance_of_scan):
        ...
        Returns:
            Nothing
        """
        self.get_logger("starting constant sweep")
        self.set_new_range(distance_of_scan)
        self.get_logger(f"stream: {self.stream}")

        # Scan indefinitely
        while not self.is_shutdown():
            try:
                self.get_logger(f"starting sweep from {start_angle} to {end_angle}")
                sonar_sweep = self.get_sweep(start_angle, end_angle)
                self.get_logger("finishng sweep")
                if self.stream:
                    compressed_image = self.convert_to_ros_compressed_img(sonar_sweep)
                    self.sonar_image_publisher.publish(compressed_image)
            except Exception as e:
                self.signal_shutdown(f"Shutting down sonar node {e}.")
    ...
    def preform_sonar_request(self):
        ...
        if left_gradians < 0 or right_gradians < 0 or right_gradians > 400 or new_range < 0:
            self.get_logger("Bad sonar request")
            return
        ...
        if object_pose is None:
            self.get_logger("No object found")
            response.pose = Pose()
    ...
    def run(self):
        ...
        if self.debug:
            ...
            self.create_subscriber(SonarSweepRequest, self.SONAR_REQUEST_TOPIC,  self.on_sonar_request)
            rate = self.Rate(5)
            while not self.is_shutdown():
...
if __name__ == '__main__':
    try:
        Sonar().run()
    except self.ROSInterruptException:
        pass
